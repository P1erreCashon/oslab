...
你可以使用图示指令查看 initcode 可执行文件的段信息和 Entry point。 Type 栏的 Offset 参数是这个段在可执行文件中的偏移地址，我们不用关系这个参数；VirtAddr 参数是这个段应该被放到的虚拟地址，这里 LOAD 段应该被放到 0x0 处。PhysAddr 和 FileSiz 参数我们不用关心，你只用确保你在映射时应用程序的 sz 大于这个 MemSiz 参数就行了。例如这里的 MemSiz 参数是 0x550字节，一个页就够了。你还需确保映射的页拥有 Flags 参数要求的 RWX 权限。由于我们在映射时会保证页对齐，这里的 Align 参数也不用关心。
