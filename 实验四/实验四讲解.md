...
除此之外，你还需要实现 usertrap 和 usertrapret 两个函数。usertrap 和 kerneltrap 的道理类似，我们需要在真正处理用户 trap 前保存 sepc 寄存器的值以确保我们 usertrapret 时能跳转到正确的位置。

xv6 的 usertrapret 函数包含着返回用户态之前的一些 CSR 设置和应用程序 trap 上下文的设置。**只有正确完成了这些设置，我们才能正常返回用户态**。当然，返回用户态前你还需要检查一下你的应用程序的页表映射和权限是否正确（人工审查一下代码），不然一回到用户态就会出现页故障意外地进入 trap 流程，不方便调试。
