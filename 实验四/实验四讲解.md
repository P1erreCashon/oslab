...
我们将起始地址 BASE_ADDRESS 设置为 $\text{0x00000}$ ，显然它只能是一个地址空间中的虚拟地址而非物理地址。事实上由于我们将入口汇编代码段放在最低的地方，这也是整个应用的入口点。我们只需清楚这一事实即可，而无需像之前一样将其硬编码到代码中。由于实验四中我们的用户代码比较少，而且用户代码没有用到 .data .rodata .bss 段，你只需要在本实验中映射一页作为 .text 段就可以了。
...
按照代码的执行顺序，实验四中你首先要实现从内核启动通过**任务切换**到第一个应用程序的过程。你需要在代码中增加 **任务相关定义**，并准备第一个应用程序初始化的相关函数。例如以下代码：
...
在本实验的最后我们还会详细讲一下怎么初始化我们的第一个应用程序，这里你只用留个印象就行了。
...
对于当前正在执行的任务的 Trap 控制流，我们用一个名为 old的变量来保存放置当前任务上下文的地址；而用 new 的变量来保存放置下一个要执行任务的上下文的地址。利用 C 语言的引用来描述的话就是：

context *old = &mycpu()->context;

context *new   = &proczero.context;

接下来我们同样从栈上内容的角度来看 swtch 的整体流程：

![switch](./images/switch.png)

Trap 控制流在调用 swtch 之前就需要明确知道即将切换到哪一条目前正处于暂停状态的 Trap 控制流，因此 swtch 有两个参数，第一个参数代表它自己，第二个参数则代表即将切换到的那条 Trap 控制流。这里我们用上面提到过的 old 和 new 作为代表 **(注：图中current_task_cx_ptr 为 old,next_task_cx_ptr 为 new)**。在上图中我们假设某次 swtch 调用要从 Trap 控制流 A 切换到 B，一共可以分为四个阶段，在每个阶段中我们都给出了 A 和 B 内核栈上的内容。
...
我们手写汇编代码来实现 swtch 。在阶段 [1] 可以看到它的函数原型中的两个参数分别是当前 A 任务上下文指针 old 和即将被切换到的 B 任务上下文指针 new ，从 RISC-V 调用规范 可以知道它们分别通过寄存器 a0/a1 传入。阶段 [2] 即将当前 CPU 状态（包括 ra 寄存器、 s0~s11 寄存器以及 sp 寄存器）保存到 A 任务上下文。相对的，阶段 [3] 即根据 B 任务上下文保存的内容来恢复上述 CPU 状态。从中我们也能够看出 context 结构 里面究竟包含哪些寄存器：
...
总结一下，实验四中你需要先实现 swtch 功能，并在你的 main 函数结尾处通过 swtch 切换到你的第一个应用程序，像下面示例一样：
...
void main()
{
    ...
    // 上下文切换
    mycpu()->proc = &proczero;
    swtch(&mycpu()->context, &proczero.context);
}
```
...
在实验四中，你需要先提供 cpu 结构的定义和 myproc() 这些函数来和 xv6 保持一致，方便以后扩展功能：
...
切换到用户程序后，他就会沿着 xv6 中 usertrapret 的逻辑返回用户态了。所有接下来我们要实现关于用户 trap 的流程。

上面的小节我们看到无论是内核还是应用的地址空间，最高的虚拟页面都是一个跳板。同时应用地址空间的次高虚拟页面还被设置为用来存放应用的 Trap 上下文。那么跳板究竟起什么作用呢？为何不直接把 Trap 上下文仍放到应用的内核栈中呢？
...
**·** kernel_hartid 用来保存内核运行在的 hart 上的 id。我们在实验中只使用单核，这里仅是和 xv6 保持一致；

**·** 最后我们保存了所有通用寄存器。

注意，

对于通用寄存器而言，两条控制流（应用程序控制流和内核控制流）运行在不同的特权级，所属的软件也可能由不同的编程语言编写，虽然在 Trap 控制流中只是会执行 Trap 处理相关的代码，但依然可能直接或间接调用很多模块，因此很难甚至不可能找出哪些寄存器无需保存。既然如此我们就只能全部保存了。但这里也有一些例外，如 x0 被硬编码为 0 ，它自然不会有变化；还有 tp(x4) 寄存器，除非我们手动出于一些特殊用途使用它，否则一般也不会被用到。虽然它们无需保存，但我们仍然在 trapframe 中为它们预留空间，主要是为了后续的实现方便。
...
Trap 在使用 C 实现的 usertrap 函数中完成分发和处理：
...
进入 usertrap 函数后，我们需要将 stvec 寄存器改成 kernelvec 来处理内核态中断，同时将 sepc 寄存器的值保存到我们的 trapframe 里。这是由于我们处理内核态中断时 sepc 的值可能会发生变化，所以和我们处理内核中断时一样，我们要在 intr_on() 开启内核 trap 之前保存好 sepc 寄存器的值，并在后面 usertrapret 时恢复 sepc 寄存器的值。

在我们的 usertrapret 函数中，我们需要在调用 userret 汇编函数前完成对一些 CSR 寄存器的设置还有对 trapframe 的设置，来保证我们能成功返回用户态以及保证后面的用户 trap 流程不出问题：
...
**·** 我们首先关闭了中断，然后把 stvec 的值设置为零 uservec 汇编的虚拟地址。

**·** 然后我们将 trapframe 中 kernel_satp 、 kernel_sp 、 kernel_trap 、kernel_hartid 的值都进行了设置，供下次进入 uservec 时使用。

**·** 由于我们可能在 usertrap 的流程中触发了 内核 trap,我们还需要把 sstatus 的 SPP 设置成 user mode。

**·** 接着我们使用我们在 usertrap 函数中保存到 trapframe 的 sepc 值设置当前 sepc 寄存器的值。这一步的原因在上面已经说明了。

**·** 最后我们就可以调用 userret 汇编函数回到用户态了。注意，这个汇编函数需要一个参数 satp 。
...
这样，这段汇编代码放在一个物理页帧中，且 uservec 恰好位于这个物理页帧的开头，其物理地址被外部符号 trampoline 标记 （这个标记是 kernel/trampoline.S 中 .globl trampoline 实现的）。在开启分页模式之后，内核和应用代码都只能看到各自的虚拟地址空间，而在它们的视角中，这段汇编代码都被放在它们各自地址空间的最高虚拟页面上，由于这段汇编代码在执行的时候涉及到地址空间切换，故而被称为跳板页面。
...
现在可以说明我们在创建用户/内核地址空间中用到的映射 trampoline 是如何实现的了：
...
设计一个应用程序时要有基本的支持功能库，这样应用程序在用户态通过操作系统提供的服务完成自身的任务。

好消息是，我们的所有实验中只用实现非常简单的应用程序，而且你应该不需要自己实现。目前我们只用实现源代码为这样的应用程序：
```
#include "sys.h"
...
你只有知道怎么使用我们的应用程序就行了。实验会给你提供一个 user 文件夹，里面有一个 Makefile 。
...
这个 Makefile 会把我们的应用程序编译出来然后提取出可执行文件中的机器码并且放到 ../include/proc/initcode.h 文件中。你可以按照以下步骤找到我们的 initcode 机器码：

**·** 进入 user 文件夹；

**·** 执行 make init 命令；

**·** 在 user 文件夹的相对路径 ../include/proc/initcode.h 处找到机器码。

initcode.h 文件中应该是这样的：
...
{   
    ...
    //目前我们的应用程序不涉及到全局变量的使用，我们只用复制程序的机器码
    //我们会在之后的实验中以加载 ELF 文件的形式执行应用程序。
